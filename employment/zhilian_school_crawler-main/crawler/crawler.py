...
def get_job_class_map():
    ...
    if os.path.exists('../crawler/persist/job_class_map_v2.json'):
        with open('../crawler/persist/job_class_map_v2.json', 'r', encoding='utf-8') as f:
            ...
            print(f"已加载专业代码映射: {job_class_map}")
...
def get_additional_info(driver):
    ...
    try:
        # 提取发布时间
        update_time_element = WebDriverWait(driver, 5).until(
            EC.presence_of_element_located((By.CLASS_NAME, "job-banner__update-time"))
        )
        update_time = update_time_element.find_element(By.TAG_NAME, "span").text
        update_time = update_time.replace("更新", "").strip()
    except TimeoutException:
        update_time = "信息暂无"
    ...
    #获取工作地点
    try:
        work_location = driver.find_element(By.CSS_SELECTOR, "div.job-info__work-location p").text
    except NoSuchElementException:
        work_location = "信息暂无"
