...
export const config: ViewConfig = { menu: { order: 4, icon: 'vaadin:records' }, title: '修改简历' };
...
const resumeRewriter = () => {
  ...
  const handleModifyResume = async () => {
    ...
    try {
      const response = await axios.post(`${apiBaseUrl}/resume/upload`, formData, {
        headers: {
  ...
  const handleDownloadResume = async () => {
    ...
    try {
      const fullUrl = `${apiBaseUrl}/${pdfUrl}`;
      console.info('pdf url:' + fullUrl);
